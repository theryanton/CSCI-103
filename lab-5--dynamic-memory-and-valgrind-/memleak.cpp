...
{
  int sum = 0;
  for(int i=0; i < len; i++){ // i should not iterate to the length of the array then there is nothing in the last index
    sum += data[i];
  }
  return sum;
}
...
char* test2_reverse(char* word)
{
  ...
  otherword[len] = '\0'; // index of null char cannot be length of str + 1, not enough indexes
  delete [] word;
  return otherword;
}
...
int main(int argc, char* argv[])
{
  ...
  if(test == 1){
   ...
   // for (int i=0; i<len; i++)
  //  {
      delete [] data; // must free memory allocated for data pointer
  //  }
  }
    ...
    //delete [] myword; Invalid free of memory
    delete [] otherword;
  }
  else {
